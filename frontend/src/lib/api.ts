...
const BASE = 'https://aigymbuddy-production.up.railway.app'
...
export const logWeight = (weight_kg: number, note?: string) => api.post('/api/users/weight', { weight_kg, note })
export const getWeightHistory = () => api.get('/api/users/weight/history')
export const generateWorkout = (params: any) => api.post('/api/workouts/generate', params)
export const completeWorkout = (workout_id: string, kcal_burned?: number) => api.post('/api/workouts/complete', { workout_id, kcal_burned })
...
export const analyseReport = (report_text: string) => api.post('/api/health/report/analyse', { report_text })
export const getLatestHealth = () => api.get('/api/health/latest')
export const chatWithCoach = (message: string, history: any[]) => api.post('/api/coach/chat', { message, history })
export const getDailyInsight = () => api.get('/api/coach/daily-insight')
export const getFormFeedback = (exercise: string, rep_count: number) => api.post('/api/coach/form-feedback', { exercise, rep_count })
